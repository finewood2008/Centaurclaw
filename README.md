# CentaurAI — All-in-One Personal Assistant Agent

<p align="center">
  <img src="public/logo.svg" alt="CentaurAI" width="120">
</p>

<p align="center">
  <strong>A 24/7 personal assistant Agent that gets things done, built by CentaurAI Team</strong>
</p>
...
**CentaurAI** is an all-in-one personal assistant Agent developed by CentaurAI Team. It works around the clock to handle your everyday tasks — data analysis, making presentations, generating videos, writing documents, searching the web, sending emails, scheduling tasks, and more.
...
git clone https://github.com/finewood2008/Centaurclaw.git
cd Centaurclaw
...
CentaurAI can use [OpenClaw](https://github.com/openclaw/openclaw) as its agent engine.
...
The bundled runtime is interpreter-focused and does not preinstall CentaurAI skill Python packages; those can be installed at runtime on demand.
...
- `CENTAURAI_PORTABLE_PYTHON_ARCHIVE`: Local prebuilt runtime archive path (recommended for offline CI/CD)
- `CENTAURAI_PORTABLE_PYTHON_URL`: Download URL for the prebuilt runtime archive
- `CENTAURAI_WINDOWS_EMBED_PYTHON_VERSION` / `CENTAURAI_WINDOWS_EMBED_PYTHON_URL` / `CENTAURAI_WINDOWS_GET_PIP_URL`: Optional overrides for Windows-host bootstrap sources

## Architecture

CentaurAI uses Electron's strict process isolation. All cross-process communication goes through IPC.
...
Cowork is the core feature of CentaurAI — an AI working session system built on the Claude Agent SDK. Designed for productivity scenarios, it can autonomously complete complex tasks like data analysis, document generation, and information retrieval.
...
CentaurAI ships with 16 built-in skills covering productivity, creative, and automation scenarios, configured via `SKILLs/skills.config.json`:
...
CentaurAI supports scheduled tasks that let the Agent automatically execute recurring work on a set schedule.
...
CentaurAI can bridge the Agent to multiple IM platforms. Send a message from your phone via IM to remotely trigger the desktop Agent — command your personal assistant anytime, anywhere.
...
CentaurAI has a built-in memory system that remembers your personal information and preferences across sessions, making the Agent more helpful the more you use it.
...
- **Explicit Requests** — Tell the Agent directly, e.g., "remember that I prefer Markdown format" or "note down that my project is called CentaurAI," and it will store the memory with higher confidence
...
All data is stored in a local SQLite database (`centaurai.sqlite` in the user data directory).
...
CentaurAI enforces security at multiple layers:
...
CentaurAI pins its OpenClaw dependency to a specific release version, declared in `package.json`:
...
[![Star History Chart](https://api.star-history.com/svg?repos=finewood2008/Centaurclaw&type=date&legend=top-left)](https://www.star-history.com/#finewood2008/Centaurclaw&type=date&legend=top-left)

---

Built and maintained by CentaurAI Team.
