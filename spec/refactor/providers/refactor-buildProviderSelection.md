...
`buildProviderSelection`（`src/main/libs/openclawConfigSync.ts`，搜索 `const buildProviderSelection` 定位）负责将 CentaurAI 的 provider 配置映射为 OpenClaw 的 `OpenClawProviderSelection` 结构。当前存在三个核心问题：
...
| Provider | 处理方式 | 状态 |
|----------|----------|------|
| `centaurai-server` | 专用分支（token proxy） | ✅ 已覆盖 |
...
| 类别 | 散落的字符串 | 出现位置 |
|------|-------------|---------|
| Provider Name | `'centaurai-server'`, `'moonshot'`, `'gemini'` 等 | `buildProviderSelection` 的 if 条件、`claudeSettings.ts` 的 `getEffectiveProviderApiFormat`、renderer `config.ts` 的 key |
...
```typescript
// ─── Provider Name ──────────────────────────────────────────────────────
// providerName 用于标识 CentaurAI 内部的 provider（对应 config key）
...
type ProviderDescriptor = {
   ...
   * Most providers use env var placeholders; centaurai-server may use 'proxy-managed'.
...
### Step 3: 保留 `centaurai-server` 的日志

原函数中 `centaurai-server` 分支有 `console.log` 输出调试信息（搜索 `buildProviderSelection centaurai-server` 定位），需在 descriptor 的 `normalizeBaseUrl` 或核心函数中保留。

**QA**（通过 Step 4 的测试验证，运行 `npm test -- openclawConfigSync`）:
1. 测试用例 `'centaurai-server logs proxy info'`：使用 `vi.spyOn(console, 'log')` 断言当 `providerName === 'centaurai-server'` 时，`console.log` 被调用且参数包含 `'[OpenClawConfigSync]'` 标签。
...
```typescript
// 场景 1: centaurai-server（token proxy 模式）
// 输入: providerName='centaurai-server', baseURL='http://example.com/v1/chat/completions', apiType='openai'
// 断言:
//   providerId === 'centaurai-server'
