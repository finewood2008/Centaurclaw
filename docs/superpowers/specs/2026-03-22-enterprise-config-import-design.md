...
CentaurAI needs enterprise batch deployment support: IT/ops teams push a unified config file to multiple machines, and CentaurAI loads it on startup. The config file reuses OpenClaw's `openclaw.json` format so that ops teams maintain a single config for both OpenClaw and CentaurAI deployments.

## Usage

```bash
centaurai --config /opt/centaurai/enterprise.json
```
...
```
Startup: centaurai --config /path/to/config.json
  │
...
OpenClaw uses plugin-style channel keys that differ from CentaurAI's internal platform names. The importer accepts **both** forms for flexibility:

| openclaw.json channel key | CentaurAI im_config key |
|---------------------------|------------------------|
...
```
[ConfigImporter] loading enterprise config from /opt/centaurai/enterprise.json
