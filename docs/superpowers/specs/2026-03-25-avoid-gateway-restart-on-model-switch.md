...
Eliminate OpenClaw gateway process restarts when users switch between 套餐模型 (centaurai-server) and 自定义模型 (custom providers), or between different custom providers with different apiKeys.
...
resolveAllProviderApiKeys() ──────────────────────────┐
  (claudeSettings.ts)                                  │
  Enumerates all enabled providers + centaurai-server   │
...
|----------|-------------|--------|
| centaurai-server | `LOBSTER_APIKEY_SERVER` | accessToken from auth |
| moonshot | `LOBSTER_APIKEY_MOONSHOT` | provider config apiKey |
| anthropic | `LOBSTER_APIKEY_ANTHROPIC` | provider config apiKey |
| ollama | `LOBSTER_APIKEY_OLLAMA` | `sk-centaurai-local` (no key needed) |
...
For centaurai-server, hardcoded as `SERVER` (since it's a dynamic provider, not in app_config.providers).
...
2. `buildProviderSelection()` — all 4 cases updated:
   - centaurai-server: `${LOBSTER_APIKEY_SERVER}` (was inline apiKey)
