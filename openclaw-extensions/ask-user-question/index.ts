 ...
 * and waits for the user's response via an HTTP callback to CentaurAI.
 *
 * This enables delete-confirmation modals on the CentaurAI desktop app
...
const plugin = {
  ...
  description: 'Structured user confirmation tool for CentaurAI desktop.',
  configSchema: {
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.registerTool((ctx) => {
      // Only enable for CentaurAI desktop sessions (sessionKey starts with 'agent:main:centaurai:').
      ...
      const isLocalDesktop = sessionKey.startsWith('agent:main:centaurai:');
      if (!isLocalDesktop) {
        return null;
      }
